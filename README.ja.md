...
Autosubmit は保存済みの `submit` 引数を定期的に自動実行します。
...
`--no-spinner` は引き続きトップレベルの CLI フラグです。正しい書き方は
`tokscale --no-spinner autosubmit status` であり、
`tokscale autosubmit status --no-spinner` はサポートしていません。
